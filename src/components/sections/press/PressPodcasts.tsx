        ...
        >
          {ROLNICK_PODCASTS.map((show) => (
            <motion.li
              key={show.name}
              variants={fadeUp}
              className="flex flex-col items-center text-center rounded-lg border border-line bg-cream p-5 shadow-[0_4px_14px_-8px_rgba(25,55,99,0.18)] transition hover:border-accent/40 hover:shadow-[0_18px_40px_-22px_rgba(25,55,99,0.32)]"
            >
              ...
              <p className="mt-4 font-display text-sm text-navy leading-tight">{show.name}</p>
            </motion.li>
          ))}
        </motion.ul>
      </div>
